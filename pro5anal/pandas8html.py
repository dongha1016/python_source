...
res.raise_for_status()    # 요청 실패 시
print(res.text, type(res.text))     # xml 모양의 <class 'str>

root = etree.fromstring(res.text)    
print(root)               # <Element '{current}current' at 0x000002B934C03B50>

print("{cuurent} namespace 제거")
for elem in root.iter():
    if '}' in elem.tag:
        elem.tag = elem.tag.split('}', 1)[1]    # '}'를 기준으로 잘라 태그명만 남김
#{Currenct}Weather => Weather 
